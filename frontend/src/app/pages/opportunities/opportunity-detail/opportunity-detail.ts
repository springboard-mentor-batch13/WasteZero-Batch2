...
export class OpportunityDetail implements OnInit {
    ...
    this.route.paramMap.subscribe((params) => {
      ...
      this.opportunityService.getById(id).subscribe({
        next: (opportunity) => {
          ...
          if (this.isVolunteer()) {
            this.loadApplicationState(opportunity.id);
          }
  ...
  delete(): void {
    if (!this.opportunity || !this.canManageSelectedOpportunity()) {
      return;
    }
  ...
  apply(): void {
    ...
    if (!user?.id) {
      this.snackBar.open(
        'Unable to submit application. Please sign in again.',
        'Close',
        { duration: 3500 }
      );
    ...
    this.applicationService.apply(request).subscribe({
      next: () => {
        this.appliedOpportunityIds.add(
  ...
  hasJoinRequest(opportunityId: string): boolean {
    return this.appliedOpportunityIds.has(
      opportunityId
    );
  }

  joinButtonText(opportunityId: string): string {
    if (this.sendingJoinRequest) {
      return 'Sending...';
    }

    if (this.hasJoinRequest(opportunityId)) {
  ...
  messageOpportunityCreator(): void {
    if (!this.opportunity) {
      return;
    }
    ...
    const postedBy = this.opportunity.postedBy;

    if (
      typeof postedBy === 'object' &&
      postedBy?._id
    ) {
      creatorId = String(postedBy._id);
    }

    if (
      !creatorId &&
      this.opportunity.ngoId
    ) {
      creatorId = String(this.opportunity.ngoId);
    }

    if (!creatorId) {
      this.snackBar.open(
        'Unable to find the NGO who created this opportunity.',
        'Close',
        { duration: 3500 }
      );
      ...
      .subscribe({
        next: (result) => {
          if (
            ...
            this.appliedOpportunityIds.delete(
              opportunityId
            );
